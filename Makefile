...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=acorn \
	-X github.com/cosmos/cosmos-sdk/version.AppName=acornd \
...
install: validate-go-version go.sum
	@echo "--> Installing acornd"
	@echo "go install -mod=readonly $(BUILD_FLAGS) ./cmd/acornd"
	@go install -mod=readonly $(BUILD_FLAGS) ./cmd/acornd

build: validate-go-version go.sum
	@echo "--> Build acornd"
	@go build -mod=readonly $(BUILD_FLAGS) -o ./build/acornd ./cmd/acornd
...
release:
	...
	$(DOCKER) run \
		...
		-v `pwd`:/go/src/github.com/acorn-chain/acorn \
		-w /go/src/github.com/acorn-chain/acorn \

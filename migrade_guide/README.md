...
mkdir acorn45
cd acorn45
git clone https://github.com/acorn-chain/acorn.git
cd acorn
git checkout dev

# install dependency and build new acornd, this file acornd will stay in current directory
make build
```
...
```
# query proposal
acornd q gov proposals

# deposit to proposal
acornd tx gov deposit <proposal_id> <deposit_amount> --from <key_name> --yes --fees 20uacorn --chain-id acorn-testnet

# vote for proposal
acornd tx gov vote <proposal_id> yes --from <key_name> --yes --fees 20uacorn --chain-id acorn-testnet

```

- After proposal is passed, the network will halt once a pre-defined upgrade block height has been reached.
- Stop current acornd daemon.
- Start new acornd daemon.

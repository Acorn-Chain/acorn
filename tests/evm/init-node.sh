...
echo "$VAL_MNEMONIC" | acornd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"

# Store the validator address in a variable to use it later
node_address=$(acornd keys show -a "$VAL_KEY")

echo "$USER1_MNEMONIC" | acornd keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER2_MNEMONIC" | acornd keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER3_MNEMONIC" | acornd keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER4_MNEMONIC" | acornd keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
...
acornd add-genesis-account "$(acornd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING")" "1000000000uacorn,100000000$IBC_DENOM" --keyring-backend "$KEYRING"
acornd add-genesis-account "$(acornd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING")" 1000000000uacorn --keyring-backend "$KEYRING"
...
## 1. Back to `acornd keys add` step, init more keys
## 2. Back to `acornd add-genesis-account` step, add balance for those
## 3. Clone this ~/.acornd home directory into some others, let's say `~/.clonedacornd`
## 4. Run `gentx` in each of those folders
## 5. Copy the `gentx-*` folders under `~/.clonedacornd/config/gentx/` folders into the original `~/.acornd/config/gentx`

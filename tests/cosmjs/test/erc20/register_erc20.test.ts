...
import { evmos, cosmos } from '@acorn-chain/acornjs';

import hre from "hardhat";
import { assert } from 'chai';

import { convertBech32AddressToEthAddress } from '../util/convert_address';
import { USERS, setupClients, localacorn } from '../util/test_setup';
...
describe('Should work with ERC20 tokens', () => {
  before(async () => {
    const testClients = await setupClients('localacorn');
    ...
    const receiver = convertBech32AddressToEthAddress('acorn', cosmosAccount.address)
  ...
  it('can register an ERC20 token', async () => {
    ...
    const proposalMsg = cosmos.gov.v1beta1.MsgSubmitProposal.fromPartial({
      ...
      // authority: "acorn10d07y265gmmuvt4z0w9aw880jnsr700jp5y852"
    })

    const fee = {
      amount: [{ amount: '200000', denom: 'uacorn' }],
      gas: '400000'
    } as StdFee
    ...
    const convertFee = {
      amount: [{ amount: '500000', denom: 'uacorn' }],
      gas: '1000000'
    } as StdFee
    const sender = convertBech32AddressToEthAddress('acorn', account.address)

...
describe('Bank', () => {
  ...
  it('should send tokens from a cosmos address to cosmos address', async () => {
    ...
    const prevBalance = await cosmosClients[1].getBalance(recipient, 'uacorn');

    const fee = {
      amount: [{ amount: '100000', denom: 'uacorn' }],
      gas: '200000'
    } as StdFee
    await cosmosClients[0].sendTokens(account.address, recipient, amount, fee);

    const afterBalance = await cosmosClients[1].getBalance(recipient, 'uacorn');
  ...
  it('should send tokens from a cosmos address to evm address', async () => {
    ...
    const recipient = convertEthAddressToBech32Address('acorn', evmAccount);

    const prevBalance = await publicClient.getBalance({
      address: evmAccount,
    });

    // 1 Acorn, should see 1 Acorn in the EVM account
    const amount = [
      {
        denom: 'uacorn',
        amount: '1000000',
